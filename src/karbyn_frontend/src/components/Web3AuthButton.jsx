import React from 'react';
import { useWeb3AuthConnect, useWeb3AuthUser, useWeb3AuthDisconnect } from "@web3auth/modal/react";

const Web3AuthButton = ({
  className = '',
  variant = 'primary',
  size = 'md',
  children
}) => {
  const { connect, loading: connectLoading, error: connectError } = useWeb3AuthConnect();
  const { disconnect, loading: disconnectLoading } = useWeb3AuthDisconnect();
  const { user, isConnected } = useWeb3AuthUser();

  const baseClasses = 'font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed';
  
  const variants = {
    primary: 'bg-gradient-to-r from-green-600 to-green-700 text-white hover:from-green-700 hover:to-green-800 focus:ring-green-500 shadow-lg hover:shadow-xl',
    secondary: 'bg-white text-green-700 border-2 border-green-600 hover:bg-green-50 focus:ring-green-500'
  };

  const sizes = {
    sm: 'px-4 py-2 text-sm',
    md: 'px-6 py-3 text-base',
    lg: 'px-8 py-4 text-lg'
  };

  const buttonClasses = `${baseClasses} ${variants[variant]} ${sizes[size]} ${className}`;
  const isLoading = connectLoading || disconnectLoading;

  const handleClick = async () => {
    try {
      if (isConnected) {
        await disconnect();
      } else {
        await connect();
      }
    } catch (error) {
      console.error('Web3Auth action failed:', error);
    }
  };

  const renderButtonContent = () => {
    if (isLoading) {
      return (
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          <span>{isConnected ? 'Signing out...' : 'Signing in...'}</span>
        </div>
      );
    }

    if (isConnected && user) {
      return (
        <div className="flex items-center space-x-3">
          {user.profileImage && (
            <img 
              src={user.profileImage} 
              alt="Profile" 
              className="w-6 h-6 rounded-full"
            />
          )}
          <span>
            {children || `Welcome, ${user.name || user.email || 'User'}!`}
          </span>
        </div>
      );
    }

    return (
      <div className="flex items-center space-x-2">
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path 
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" 
            fill="#4285F4"
          />
          <path 
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" 
            fill="#34A853"
          />
          <path 
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" 
            fill="#FBBC05"
          />
          <path 
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" 
            fill="#EA4335"
          />
        </svg>
        <span>{children || 'Sign in with Google'}</span>
      </div>
    );
  };

  return (
    <div className="space-y-2">
      <button 
        onClick={handleClick}
        disabled={isLoading}
        className={buttonClasses}
      >
        {renderButtonContent()}
      </button>
      
      {connectError && (
        <div className="text-red-600 text-sm bg-red-50 p-2 rounded-lg">
          {connectError.message}
        </div>
      )}
    </div>
  );
};

export default Web3AuthButton;
